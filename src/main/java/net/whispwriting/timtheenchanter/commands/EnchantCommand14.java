...
public class EnchantCommand14 implements CommandExecutor {
                    ...
                    try {
                        ...
                        if (player.hasPermission("TimTheEnchanter.enchant.unsafe")) {
                            if (level > 1000) {
                                ...
                                if (args[0].equalsIgnoreCase("quick_charge") && level > 5) {
                                    item.addUnsafeEnchantment(Enchantment.getByKey(NamespacedKey.minecraft(args[0])), 5);
                                } else {
                                ...
                                try {
                                    ...
                                    if (enchNameLow.equals("quick charge")){
                                        if (level > 5){
                                            level = 5;
                                        }
                                    }
    ...
    private void enchAll(Player player, ItemStack item){
        ...
        if (player.hasPermission("TimTheEnchanter.enchant.unsafe") && !safeEnch){
            for (int i=0; i<enchants.size(); i++) {
                if (enchants.get(i).getKey().toString().equals("minecraft:quick_charge")){
                    item.addUnsafeEnchantment(enchants.get(i), 5);
                }else {
                ...
                try {
                    ...
                    for (int k = 0; k < lore.size(); k++) {
                        if (lore.get(k).contains(enchName)) {
                            if (enchName.contains("Curse")){
                                ...
                                if (enchName.equals("Quick Charge ")){
                                    lore.set(k, ChatColor.GRAY + "" + ChatColor.ITALIC + enchName + "V");
                                }else {
                    ...
                    if (enchName.contains("Curse")){
                        ...
                        if (enchName.equals("Quick Charge ")){
                            lore.add(ChatColor.GRAY + "" + ChatColor.ITALIC + enchName + "V");
                        }else {
                    ...
                    for (int m = 0; m < lore.size(); m++) {
                        if (lore.get(m).contains(enchName)) {
                            if (enchName.contains("Curse")){
                                ...
                                if (enchName.equals("Quick Charge ")){
                                    lore.set(m, ChatColor.GRAY + "" + ChatColor.ITALIC + enchName + "V");
                                }else {
